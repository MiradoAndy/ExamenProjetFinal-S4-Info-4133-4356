...
[x]-V2
        [x] -Coté opérateur (MIRADO)
                [x] -Configuration des préfixes valable pour les autres opérateurs (MIRADO)
                        [x] -un formulaire de saisie des préfixes (MIRADO)
                        [x] -validation des préfixes (MIRADO)
                        [x] -inclure une nouvelle table pour les préfixes des autres opérateurs (MIRADO)
                [x] -Configuration % en plus de commissions pour les transferts vers les autres opérateurs 
                        [x] -ajouter un frais en plus pour les transferts vers les autres opérateurs (MIRADO)
                        [x] -un formulaire de saisie du % en plus de commissions (MIRADO)
                                [x] -dropdown pour choisir si la valeur choisie est un pourcentage ou un montant fixe (MIRADO)
                        [x] -validation du % en plus de commissions (MIRADO)
                [x] -Situation gain via les différents frais (MIRADO)
                        [x] -séparer les gains obtenu de l'operateur des gains des autres operateurs (MIRADO)
                [x] -Situation des montants à envoyer à chaque opérateur (MIRADO)
                        [x] -front (MIRADO)
                                [x] -dashboard affichant le montant total à envoyer à chaque opérateur (MIRADO)
                        [x] -back (MIRADO)
                                [x] -fonction qui retourne le montant total à envoyer à chaque opérateur (MIRADO)
        [x] -Coté client (AINA)
                [x] -option inclure frais de retrait lors de l'envoi (AINA)
                        [x] -front (AINA)
                                [x] -checkbox pour inclure les frais de retrait lors de l'envoi (AINA)
                        [x] -back (AINA)
                                [x] -si inclu: l'expediteur paie montant + frais de transfert + frais de retrait (AINA)
                                [x] -si pas inclu: le destinataire paie montant + frais de retrait (AINA)
                [x] -envoie multiple vers plusieurs numero (AINA)
                        [x] -front (AINA)
